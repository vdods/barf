...
void EmitConflictWarning (string const &message)
{
    ...
    if (GetTrisonOptions().GetTreatWarningsAsErrors())
        cerr << GetTrisonOptions().GetInputFilename() << ": error: " << message << endl;
    else
        cerr << GetTrisonOptions().GetInputFilename() << ": warning: " << message << endl;
}
